...
# --------------------------------------------------
def query_GEE_for_Landsat_SR(AOI, date_start, date_end, month_start, month_end, cloud_cover_max, dataset_dict):
    ...
    # convert image collection to xarray Dataset in UTM CRS
    L_xr = L_clip.wx.to_xarray(scale=30, crs='EPSG:'+epsg_UTM)
    
    # replace no data values with NaN
    for band in L_band_names:
        L_xr[band] = L_xr[band].where(L_xr[band] != dataset_dict['no_data_value'])
    # account for image scalar
    for band in L_band_names[0:-1]:
        L_xr[band] = L_xr[band] * dataset_dict['SR_scalar']
        
    # Add NDSI band
    NDSI_bands = dataset_dict['NDSI']
...
# --------------------------------------------------
#def classify_image_collection(im_collection, clf, feature_cols, crop_to_AOI, AOI, out_path, plot_results):
#    '''
#    Function to classify Landsat image collection using a pre-trained classifier
#
#    Parameters
#    ----------
...
#        im_classified_fn = im_date + "_classified.tif"
#        if os.path.exists(os.path.join(out_path, im_classified_fn)):
#            print('classified image already exists in file, skipping...')
#            continue
#
...
#            # apply mask to bands
#            for band in bands:
#                im[band] = im[band].where(mask==1)
#
#        # find indices of real numbers (no NaNs allowed in classification)
#        ix = [np.where(np.isnan(im[band].data), False, True) for band in bands]
#        I_real = np.full(np.shape(im[bands[0]].data), True)
#        for ixx in ix:
#            I_real = I_real & ixx
#
...
# --------------------------------------------------
def Landsat_mask_clouds(im_collection, AOI, plot_results):
    '''
...
#             'clear': 1<<6,
    ...
    for i, t in enumerate(im_collection.time):
        ...
        # -----Skip image if it contains < 75% of the AOI
        # clip image to AOI
        ...
        if p_real < 0.50:
            ...
            print('image contains ' + str(np.round(p_real*100)) + '% real values in the AOI, continuing...')
            ...
            if plot_results:
                ...
                # masked image
                ax[1].imshow(np.dstack([im_mask['SR_B4'].data, im_mask['SR_B3'].data, im_mask['SR_B2'].data]),
                    extent=(np.min(im.x.data)/1e3, np.max(im.x.data)/1e3,
                                              np.min(im.y.data)/1e3, np.max(im.y.data)/1e3))
                ax[1].set_xlabel('Easting [km]')
...
# --------------------------------------------------
def classify_image_collection(im_collection, clf, feature_cols, crop_to_AOI, AOI, ds_dict, out_path, date_start, date_end, plot_results, figures_out_path):
    '''
    Function to classify Landsat image collection using a pre-trained classifier
    ...
    Returns
    ----------
    L_mask_classified: xarray.Dataset
    
    L_mask_classified_fn: str
    ...
    L_mask_classified_fn = 'Landsat_SR_' + date_start.replace('-','') + '_' + date_end.replace('-','') + '_masked_classified.nc'
    if os.path.exists(os.path.join(out_path, L_mask_classified_fn)):
        print('classified image collection already exists in file, loading...')
        L_mask_classified = xr.open_dataset(out_path + L_mask_classified_fn)
        fig = 'N/A'
        ...
        for i, t in enumerate(im_collection.time):
            ...
            if plot_results:
                ...
                fig_fn = figures_out_path + 'Landsat_' + str(t.data)[0:10] + '_SCA.png'
            ...
            if im_count==0:
                ...
                L_mask_classified = xr.Dataset(
                    ...
                    attrs=im.attrs)
                # add time dimension
                L_mask_classified.expand_dims(dim={"time": [im_collection.time.data[i]]})
                
            else:
                # create xarray.Dataset for image
                im_mask_classified = xr.Dataset(
                    data_vars=dict(
                        classified=(["y", "x"], im_classified)
                    ),
                    coords=im.coords,
                    attrs=im.attrs)
                # add time dimension
                im_mask_classified.expand_dims(dim={"time": [im_collection.time.data[i]]})
                # concatenate to classified image collection
                L_mask_classified = xr.concat([L_mask_classified, im_mask_classified], dim='time')
                
            im_count+=1
            
        # save classified image collection to netCDF file
        L_mask_classified.to_netcdf(out_path + L_mask_classified_fn, mode='w')
        print('Landsat classified image collection saved to file: ' + out_path + L_mask_classified_fn)
    
    return L_mask_classified, L_mask_classified_fn, fig
    
# --------------------------------------------------
def Landsat_delineate_snow_line(im_collection, im_collection_classified, date_start, date_end, site_name, AOI, DEM, out_path, figures_out_path, plot_results):
    '''
    Parameters
    ----------
    im_collection:
    ...
    if os.path.exists(os.path.join(out_path, snowlines_fn)):
        ...
        for i, t in enumerate(im_collection.time):
            ...
            # generate and filter binary snow matrix
            # create binary snow matrix
            im_binary = xr.where(im_classified_adj  > 2, 1, 0).classification.data
            # apply median filter to binary image with kernel_size of 33 pixels (~99 m)
            im_binary_filt = medfilt(im_binary, kernel_size=33)
            # fill holes in binary image (0s within 1s = 1)
            im_binary_filt_no_holes = binary_fill_holes(im_binary_filt)
            ...
            if plot_results:
                ...
                ax[0].legend(loc='best')
                ax[1].legend(loc='best')
                fig.suptitle(str(t)[0:10])
                plt.show()
                # save figure
                fig_fn = figures_out_path + 'Landsat_' + str(t)[0:10] + '_snowline.png'
