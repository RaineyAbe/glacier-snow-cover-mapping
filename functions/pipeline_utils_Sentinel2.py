# Functions for image adjustment and snow classification in Sentinel-2 surface reflectance imagery
# Rainey Aberle
# 2022
...
def query_GEE_for_DEM(AOI):
    ...
    DEM_ds = DEM.wx.to_xarray(scale=ds_dict['resolution_m'], crs='EPSG:'+str(epsg_UTM))
...
def query_GEE_for_Sentinel2_SR(AOI, date_start, date_end, month_start, month_end, cloud_cover_max, ds_dict):
    ...
    S_clip = S.map(clip_image).select(S_band_names)
    # convert image collection to xarray Dataset
    S_xr = S_clip.wx.to_xarray(scale=10, crs='EPSG:4326')
    # transform to UTM CRS
    S_xr = S_xr.rio.reproject('EPSG:'+epsg_UTM)
    # replace no data values with NaN
    for band in S_band_names:
        S_xr[band] = S_xr[band].where(S_xr[band] != ds_dict['no_data_value'])
    # account for image scalar
    for band in S_band_names[0:-1]:
        S_xr[band] = S_xr[band] * ds_dict['SR_scalar']
    # Add NDSI band
    NDSI_bands = ds_dict['NDSI']
    S_xr['NDSI'] = ((S_xr[NDSI_bands[0]] - S_xr[NDSI_bands[1]]) / (S_xr[NDSI_bands[0]] + S_xr[NDSI_bands[1]]))
...
# --------------------------------------------------
def Landsat_delineate_snow_line(im_collection, im_collection_classified, date_start, date_end, site_name, AOI, DEM, ds_dict, dataset, out_path, figures_out_path, plot_results):
    '''
    Parameters
    ----------
    im_collection:
    ...
    if os.path.exists(os.path.join(out_path, snowlines_fn)):
        ...
        for i, t in enumerate(im_collection.time):
            ...
            if plot_results:
                ...
                ax[1].legend(loc='best')
                fig.suptitle(site_name + ' Glacier: ' + im_date)
                fig.tight_layout()
                plt.show()
                # save figure
