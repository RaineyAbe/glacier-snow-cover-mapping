...
# DEM file name
DEM_fn = None #'Gulkana_20210919_USGS_DEM.tif'
...
if 1 in steps_to_run:
    ...
    im_list = f.query_GEE_for_imagery(dataset, dataset_dict, AOI, date_start, date_end, month_start,
        ...
        for i in tqdm(range(0, len(im_list))):
            ...
            if os.path.exists(im_classified_path + im_classified_fn):
                ...
                im_classified = f.classify_image(im_xr, clf, feature_cols, crop_to_AOI, AOI_UTM, DEM,
                                                 dataset, dataset_dict, site_name, im_classified_fn, im_classified_path)
...
if 3 in steps_to_run:
    ...
    dataset = 'Landsat'
    im_list = f.query_GEE_for_imagery(dataset, dataset_dict, AOI_UTM, date_start, date_end, month_start, month_end,
...
if 4 in steps_to_run:
    ...
    if mask_clouds:
        f.PlanetScope_mosaic_images_by_date(PS_im_masked_path, im_masked_fns, PS_im_mosaics_path, AOI_UTM, plot_results)
        print(' ')
    else:
        f.PlanetScope_mosaic_images_by_date(PS_im_path, im_fns, PS_im_mosaics_path, AOI_UTM, plot_results)
        print(' ')
    ...
    for im_mosaic_fn in tqdm(im_mosaic_fns):
        ...
        # -----Adjust radiometry
        im_adj, im_adj_method = f.PlanetScope_adjust_image_radiometry(im_da, im_dt, polygons_top, polygons_bottom, AOI_UTM,
                                                                      dataset_dict, dataset, site_name, skip_clipped)
