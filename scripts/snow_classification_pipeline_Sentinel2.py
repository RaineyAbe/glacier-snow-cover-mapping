...
if 1 in steps_to_run:
    ...
    plot_results=False
    S2_ds_fns = f.query_GEE_for_Sentinel2_SR(dataset, site_name, AOI, date_start, date_end, month_start, month_end, cloud_cover_max, ds_dict, im_masked_path, plot_results)
...
if 2 in steps_to_run:
    ...
    for im_masked_fn in im_masked_fns:
        ...
        # classify images
        plot_results=False
        im_classified = f.classify_image(im_masked, clf, feature_cols, crop_to_AOI, AOI, ds_dict, dataset, site_name, im_classified_path, plot_results, figures_out_path)
        print(' ')
...
if 3 in steps_to_run:
    ...
    for im_classified_fn in im_classified_fns:
        ...
        if os.path.exists(os.path.join(snowlines_path, snowline_fn)):
            ...
            im_masked_fn = [x for x in im_masked_fns if (im_dt in x)][0]
            im_masked = xr.open_dataset(im_masked_fn)
            # delineate snowline
            plot_results=True
            snowline_df = f.delineate_im_snowline(im_masked, im_classified, site_name, AOI, DEM, ds_dict,
    ...
    fig_fns_str = site_name + '_Sentinel2_*snowline.png'
    # define the output .gif filename
    gif_fn = site_name + '_Sentinel2_' + date_start.replace('-','') + '_' + date_end.replace('-','') + '_snowlines.gif'
